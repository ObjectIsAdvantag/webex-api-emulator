...
MembershipStorage.prototype.create = function (actor, roomId, newMemberId, isModerator, cb) {
   ...
   // Moderation not supported by the Emulator
   ...
   self.datastore.people.find(actor, newMemberId, function (err, person) {
      if (err) {
      ...
      // Retreive detailed room info
      self.datastore.rooms.find(actor, roomId, function (err, room) {
         if (err) {
            debug(`details not found for room: ${roomId}`);
            if (cb) {
               var err2 = new Error("details not found for specified room");
               err2.code = "PERSON_NOT_FOUND";
               cb(err2, null);
            }
            return;
         ...
         // Create membership
         const membership = self.datastore.memberships._add(actor.id, room, person);
...
MembershipStorage.prototype._add = function (actorId, room, newMember) {
